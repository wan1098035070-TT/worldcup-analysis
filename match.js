...
function teamModelProfile(team) {
  ...
  const elo = baseStrength(team);
  const rankIndex = limit((95 - team.rank) / 94, 0.08, 1);
  const squadCoverage = limit(squad.length / 11, 0.62, 1);
  const injuryRisk = limit(0.04 + pseudo(`${team.name}-injury-risk`) * 0.12 - squadCoverage * 0.04, 0.02, 0.16);
  ...
  const recentGf = limit(0.85 + attack * 0.64 + formSeed * 0.62, 0.55, 2.8);
  const recentGa = limit(1.95 - defense * 0.62 + (1 - formSeed) * 0.42, 0.45, 2.45);
  return {
    elo,
    attack,
    defense,
...
function buildPoissonScoreModel() {
  ...
  return {
    ...
    factors: [
      ...
      ["近10场均进球", `${home.recentGf.toFixed(1)} : ${away.recentGf.toFixed(1)}`],
      ["近10场均失球", `${home.recentGa.toFixed(1)} : ${away.recentGa.toFixed(1)}`],
